...
const displayIssues = (issues) => {
  ...
  issues.forEach((issue) => {
    ...
    const labels = issue.labels
      .map((label) => {
        if (label === "bug")
          return `<span class="text-red-500 bg-red-100 px-2 py-1 rounded-full text-[10px] font-bold">BUG</span>`;
        if (label === "help wanted")
          return `<span class="text-orange-500 bg-orange-100 px-2 py-1 rounded-full text-[10px] font-bold">HELP WANTED</span>`;
        if (label === "enhancement")
          return `<span class="text-green-500 bg-green-100 px-2 py-1 rounded-full text-[10px] font-bold">ENHANCEMENT</span>`;
        return "";
      })
      .join("");
    ...
    // Pass only the ID to the openModal function
...
tabButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    let filtered =
      tabText === "all"
        ? allIssues
        : allIssues.filter((issue) => issue.status === tabText);
...
const handleSearch = () => {
  ...
  fetch(`${SEARCH_API_URL}${encodeURIComponent(searchText)}`)
    ...
    .catch((err) => {
      container.innerHTML = `<div class="col-span-full text-center py-20"><p class="text-red-500">Failed to load issues.</p></div>`;
    });
};
